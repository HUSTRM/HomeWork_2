...
int my_max(double input_array[])
{
	double tp_array[max_gray_level+1];
	double tp_array1[max_gray_level+1];
	int maxnum;
	for (int flag=0; flag <= max_gray_level; flag++)
	{
		tp_array1[flag] = input_array[flag];
	}
	Quickly(input_array, tp_array, 0, max_gray_level);

	for (int flag = 0; flag <= max_gray_level; flag++)
	{
		if (tp_array1[flag] ==tp_array[max_gray_level] )
		{
			maxnum = flag;
			break;
		}
...
float max_2_num(float a, float b)
{
	if (a > b)
	{
		return a;
...
float min_2_num(float a, float b)
{
	if (a > b)
	{
		return b;
